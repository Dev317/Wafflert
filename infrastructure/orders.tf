...
resource "aws_ecs_task_definition" "orders_task_definition" {
  ...
  container_definitions = jsonencode([
    {
      image     = "157077562636.dkr.ecr.us-east-2.amazonaws.com/orders_repo:latest",
      name      = "orders",
      essential = true,
      memory    = 100,
      cpu       = 1,
      environmentFiles = [
        {
          type  = "s3"
          value = "arn:aws:s3:::production-env-g1t3-bucket/orders_production.env"
        }
      ],
      portMappings = [
        {
...
resource "aws_s3_object" "orders_production_env" {
  key    = "orders_production.env"
  bucket = aws_s3_bucket.production_env_bucket.id
  source = var.orders_production_env
}
